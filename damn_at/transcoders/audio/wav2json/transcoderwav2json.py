...
from damn_at.exceptions import TranscoderException
...
class Audio2JsonTranscoder(ITranscoder):
    options = [
        IntOption(name='channels', description='Number of channels to output', default=2, min=1, max=2),
    ...
    convert_map = {
        "audio/x-wav": {"application/json": options},
    ...
    def transcode(self, dest_path, file_descr, asset_id, target_mimetype,
        ...
        file_path = expand_path_template(
        ...
        try:
            ...
            pro = subprocess.Popen(
                ["sox", file_descr.file.filename, "-t", "wav", "-r", str(options['samplerate']), tmp.name],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            out, err = pro.communicate()
            if pro.returncode != 0:
                print("Sox failed %s with error code %d!" % (file_descr.file.filename, pro.returncode),
                      out, err)
                return False
            else:
                toopen = tmp.name
